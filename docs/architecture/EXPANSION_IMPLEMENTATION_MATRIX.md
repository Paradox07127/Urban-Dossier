...
| 1.4 Sensitivity pipeline | done | Seeded 1,000-draw artifact; API score/rank intervals and histogram; atomic manifest validates 3.9.0, artifact schema/rows/SHA and all 15 input score-table snapshots with stat-driven cache invalidation; zero-overall context metrics are audited without expanding the 7,194-cell public-composite population; backend maps the production-method 95% interval to fixed 20-point public tiers, while UI and offline HTML demote the point estimate to secondary detail | Regenerate the parquet+manifest pair after any input, method or metric-registry change; never publish a tier from an unverified artifact |
| 1.5 Dataset expansion | partial | All eight raw snapshots have manifests; NYCCAS year-16 modeled NO is the first to complete hash quarantine → native 300 m raster lookup at H3 r9 → atomic ready artifact/3.9.0 manifest → MetricDefinition → correlation → 1,000-draw sensitivity → API/methodology/detail UI publication. It covers 7,413/7,414 land centroids and remains context-only at 0% overall weight | Complete the same gates for the other seven snapshots. Tract sources remain NTA-only; sampled/incomplete raw sources cannot be promoted as production metrics |
