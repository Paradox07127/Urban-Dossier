...
| 1.4 Sensitivity pipeline | done | Seeded 1,000-draw artifact; API score/rank intervals and histogram; atomic manifest validates 3.9.0, artifact schema/rows/SHA, all 19 registered H3/ZIP score-table snapshots and the ready location-index snapshot with stat-driven invalidation. Native ZIP values are repeated over the stable 7,194-cell H3 population through production's nearest-parcel ZIP resolver, with grain still disclosed as ZIP; zero-overall context metrics cannot change the composite. Backend maps the production-method 95% interval to fixed 20-point public tiers, while UI and offline HTML demote the point estimate to secondary detail | Regenerate the parquet+manifest pair after any input, resolver, method or metric-registry change; never publish a tier from an unverified artifact |
| 1.5 Dataset expansion | partial | All eight raw snapshots have manifests. NYCCAS modeled NO and DOHMH HVI are the first two to complete hash quarantine → native-grain ready artifact/3.9.0 manifest → MetricDefinition → native-grain correlation → sensitivity → API/methodology/online+offline UI publication. NYCCAS covers 7,413/7,414 land centroids; HVI preserves 184 ZCTA classes and is an individual zero-weight context indicator. Ready-only ZIP resolution was repaired so HVI and the existing EMS/Fire/Parks metrics no longer disappear | Complete or explicitly quarantine the other six snapshots. Tract sources remain NTA-only; HVI needs an explicit ZCTA→NTA rule before mapping; sampled/incomplete raw sources cannot be promoted as production metrics |
| 1.6 Public methodology page | done | Shareable `/methodology` entry renders registry-derived metric/source/vintage/grain/normalization/weight and runtime required/ready/missing dataset tables without mounting the map; `/api/methodology` and the frontend fail closed unless Node code, registry and every metric equal 3.9.0, with Python↔Node equality pinned by test | Keep every new dataset in the required/available/missing partition and add its verified vintage before publication |
...
| 3.3 Payload policy | done | Three-tier enum resolved from `URBAN_DOSSIER_PAYLOAD_POLICY`, applied at `dispatch_tool`, every filtered payload stamped; persisted JSONL audit (one line per filtered call) in the state dir; review-found leaked sample keys (`results`, `evidence_table`, `hits`, `neighbors`) closed | Per-tool typed visibility contracts remain the stricter upgrade; extend SAMPLE_KEYS with any new record-list field |
| 3.4 Intent router | partial | Deterministic four-intent router before the skill import; out_of_scope/meta_help short-circuit with iterations=0 and never touch loop/sandbox/model; router decision now in EVERY trace (short-circuit and pass-through); first-turn session persistence fixed so evidence follow-ups can actually route | ask_from_evidence vs new_analysis still receive identical agent handling -- differential behaviour is the remaining half |
| 3.5 Socrata ingestion | partial | Keyset snapshot downloader with partial-file quarantine; full Discovery traversal (3,014/3,014 assets, scroll-from-floor with a completeness gate that refuses truncated walks) and a per-snapshot freshness watermark index landed in the state catalog | Publish transaction and quarantine index remain with the promotion flow; cross-domain sources (data.cdc.gov) and non-Socrata sources (ACS summary files) are reported untracked by design |
