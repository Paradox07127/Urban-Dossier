(function () {
  ...
  let activeCenter = DEFAULT_CENTER;
  let mapLoaded = false;
  ...
  const style = {
    ...
    layers: [
      ...
      { id: 'park-fill', type: 'fill', source: 'openmaptiles', 'source-layer': 'park', paint: { 'fill-color': '#c8e6a0', 'fill-opacity': 0.5 } },
      {
        ...
        minzoom: 13,
        paint: { 'fill-color': '#d4cfc8', 'fill-opacity': 0.7, 'fill-outline-color': '#bab6ae' },
      },
      {
        ...
        minzoom: 13,
        paint: { 'fill-color': '#e44743', 'fill-opacity': 0.92, 'fill-outline-color': '#a32824' },
      },
      {
        id: 'building-3d',
        type: 'fill-extrusion',
      ...
      { id: 'road-path', type: 'line', source: 'openmaptiles', 'source-layer': 'transportation', filter: ['in', 'class', 'path', 'track'], minzoom: 14, layout: { 'line-cap': 'round' }, paint: { 'line-color': '#ccc', 'line-width': 1, 'line-dasharray': [2, 2] } },
      { id: 'search-point', type: 'circle', source: 'searchPoint', paint: { 'circle-radius': 6, 'circle-color': '#cf3a36', 'circle-stroke-width': 2, 'circle-stroke-color': '#fff' } },
      { id: 'highlight-area-outline', type: 'line', source: 'highlightArea', paint: { 'line-color': '#cf3a36', 'line-width': 1.5, 'line-opacity': 0.25 } },
    ],
  };
  ...
  function updateSearchArea(center, label) {
    if (!mapLoaded) return;
    activeCenter = center;

    const radiusPolygon = createCirclePolygon(center, SEARCH_RADIUS_KM);
    const areaSource = map.getSource('highlightArea');
    const pointSource = map.getSource('searchPoint');
  ...
  function refreshBuildingSources(center, radiusKm) {
    const highlightSource = map.getSource('highlightBuildings');
    const normal3dSource = map.getSource('normalBuildings3d');
    if (!highlightSource || !normal3dSource) return;

    const collections = collectVisibleBuildings(center, radiusKm);
    highlightSource.setData(collections.highlighted);
    normal3dSource.setData(collections.normal3d);
  }

  function collectVisibleBuildings(center, radiusKm) {
    ...
    return {
      highlighted: { type: 'FeatureCollection', features: highlighted },
      normal3d: { type: 'FeatureCollection', features: normal3d },
    };
  }
  ...
  function resolveQuery(query) {
    ...
    for (const name in NYC_LANDMARKS) {
      if (name.includes(normalized) || normalized.includes(name)) {
        return { center: NYC_LANDMARKS[name], label: toTitleCase(name) };
      }
    }
  ...
  function createCirclePolygon(center, radiusKm) {
    ...
    for (let i = 0; i <= steps; i += 1) {
      ...
      const lat2 = Math.asin(Math.sin(latRad) * Math.cos(angularDistance) + Math.cos(latRad) * Math.sin(angularDistance) * Math.cos(bearing));
      const lng2 = lngRad + Math.atan2(Math.sin(bearing) * Math.sin(angularDistance) * Math.cos(latRad), Math.cos(angularDistance) - Math.sin(latRad) * Math.sin(lat2));
  ...
  function geometryIntersectsCircle(geometry, center, radiusKm) {
    if (geometry.type === 'Polygon') return polygonIntersectsCircle(geometry.coordinates, center, radiusKm);
    if (geometry.type === 'MultiPolygon') return geometry.coordinates.some(function (polygon) { return polygonIntersectsCircle(polygon, center, radiusKm); });
    return false;
  }

  function polygonIntersectsCircle(polygon, center, radiusKm) {
  ...
  function segmentIntersectsCircle(start, end, center, radiusKm) {
  ...
  function getBoundsForCircle(center, radiusKm) {
    const lng = center[0];
    const lat = center[1];
  ...
  function pointFeature(center, label) {
    return {
  ...
  function projectToKm(point, meanLat) {
