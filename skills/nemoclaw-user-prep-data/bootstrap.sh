#!/usr/bin/env bash
# bootstrap.sh — DGX Spark (or any bare-metal) dependency bootstrap for
# nemoclaw-user-prep-data. Run once at arrival. Idempotent.
#
# Usage:
#   bash bootstrap.sh
#
...
#   2. Checks for pandas, numpy, h3, pyproj, shapely, requests
#   3. Installs any missing deps via `pip install --user`
#   4. Prints versions on success
#   5. Runs a one-shot pandas smoke test to confirm the install works
#
# Exit codes:
...
MISSING=()
for pkg in pandas numpy h3 pyproj shapely requests; do
...
# ----------------------------------------------------------------------------
# Step 4: pandas smoke test (tiny DataFrame round-trip)
# ----------------------------------------------------------------------------

log "Running pandas smoke test..."
python3 - <<'PY'
import pandas as pd, numpy as np
df = pd.DataFrame({"a": [1, 2, 3], "b": ["x", "y", None]})
assert df.shape == (3, 2), f"unexpected shape: {df.shape}"
assert int(df["a"].sum()) == 6
assert int(df["b"].isna().sum()) == 1
print(f"  pandas version: {pd.__version__}")
print(f"  numpy  version: {np.__version__}")
print("  3x2 DataFrame round-trip OK")
PY

log "All dependencies present and working."
log "You can now run:"
log "    python3 scripts/profile.py <file>"
log "    python3 scripts/clean.py <in> <out> --ops '[...]'"
log "    python3 scripts/validate.py <orig> <cleaned>"
